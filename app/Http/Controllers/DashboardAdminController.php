...
class DashboardAdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $atrasados = Chamado::whereIn('st_status', StatusChamado::GRUPO_PENDENTE)
        ...
        $porStatus = Chamado::select('st_status', DB::raw('count(*) as value'))
            ...
            ->map(function ($item) {
                return [
                    'name' => StatusChamado::label($item->st_status),
                    'value' => $item->value,
                    'color' => StatusChamado::color($item->st_status),
                ];
            });
        ...
        $tabelaGeral->getCollection()->transform(function($c) {
            $statusLabel = StatusChamado::label($c->st_status);
